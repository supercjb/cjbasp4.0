...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse; word-break:break-all" bordercolor="#000000" width="96%" STYLE="WORD-BREAK: break-all">
  <tr>
    <td width="4%" align="center" background="images/Button.gif">
    <p align="left"><font size="2">狀態</font></td>
    <td width="46%" align="center" background="images/Button.gif">
    <font size="2">簡訊主題</font></td>
    <td width="20%" align="center" background="images/Button.gif">
    <font size="2">來自</font></td>
    <td width="20%" align="center" background="images/Button.gif">
    <font size="2">寄到時間</font></td>
    <td width="10%" align="center" background="images/Button.gif">
    <font size="2">執行</font></td>
  </tr>
...
FOR SH=1 to rs.PageSize      ' 顯示資料%>
  <tr>
    <td width="5%"align="center" background="images/postbit_lightbg.gif"><font size=-1>
	<%If rs("readed")=1 Then%><img src="images/icon_folder_new_2.gif">
	<%Else%><img src="images/icon_folder_2.gif"><%End If%>
	　</td>
    <td width="47%" background="images/postbit_lightbg.gif"><font size=-1><!-- 假使沒讀過就顯示粗體1,簡訊讀過是0 -->
	<%If rs("readed")=1 Then%><A HREF="pm_detail.asp?id=<%=rs("id")%>"><b><%=rs("totitle")%></b></a>　</td>
     <%else%>
<A HREF="pm_detail.asp?id=<%=rs("id")%>"><%=rs("totitle")%></a></td>
      <%end if%>
    <td width="20%" align="center" background="images/postbit_lightbg.gif"><font size=-1><b><%=rs("fromuser")%></b>　</td>
    <td width="19%" align="center" background="images/postbit_lightbg.gif"><font size=-1>
				<%if rs11 Is Nothing Or rs11.EOF Then%>  
				<%=DateAdd("h",rstitle("servertimezone"),rs("time"))%>
				<%else%>
				<%=DateAdd("h",rstitle("servertimezone")+rs11("timezone"),rs("time"))%>
				<%end if%>　</td>
    <td width="9%" background="images/postbit_lightbg.gif">
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse; word-break:break-all" bordercolor="#000000" width="96%" STYLE="WORD-BREAK: break-all">
  <tr>
    <td width="4%" align="center" background="images/Button.gif">
    <p align="left"><font size=-1>&nbsp;狀態</td>
    <td width="54%" align="center" background="images/Button.gif">
    <font size="2">簡訊主題</font></td>
    <td width="21%" align="center" background="images/Button.gif">
    <font size="2">傳送對象</font></td>
    <td width="21%" align="center" background="images/Button.gif">
    <font size="2">發送時間</font></td>
  </tr>
 <%  Page = cint(Request("Page"))
	 myself=request.serverVariables("PATH_INFO")
...
FOR SH=1 to rs.PageSize      ' 顯示資料%>
  <tr>
	...
	<%Else%><img src="images/icon_folder_2.gif"><%End If%>
	　</td>
    <td width="55%" background="images/postbit_lightbg.gif"><font size=-1><!-- 假使沒讀過就顯示粗體1,簡訊讀過是0 -->
	<%If rs("readed")=1 Then%><A HREF="pm_detail2.asp?id=<%=rs("id")%>"><b><%=rs("totitle")%></b></a>　</td>
     <%else%>
<A HREF="pm_detail2.asp?id=<%=rs("id")%>"><%=rs("totitle")%></a></td>
      <%end if%>
    <td width="20%" align="center" background="images/postbit_lightbg.gif"><font size=-1><b><%=rs("touser")%></b>　</td>
    <td width="20%" align="center" background="images/postbit_lightbg.gif"><font size=-1>
			<%if rs11 Is Nothing Or rs11.EOF Then%>  
			<%=DateAdd("h",rstitle("servertimezone"),rs("time"))%>
			<%else%>
			<%=DateAdd("h",rstitle("servertimezone")+rs11("timezone"),rs("time"))%>
