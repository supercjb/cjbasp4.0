...
<% response.cookies("tid")="搜尋結果"%>
<!--#include file="choice.asp"--><br>
<%
...
if request("sort") <> "" then
 if not request("sort")="aa" then
   strWhere = strWhere & " tid = " & request("sort")
 elseif request("sort")="aa" then
 end if
end if

if request("tit") <> "" then
 if request("sort")="aa" then
  strWhere = strWhere & " Subject like '%" & request("tit") & "%'"
 else
  strWhere = strWhere & " and Subject like '%" & request("tit") & "%'"
 end if
end if

if request("name") <> "" then
 if request("sort")="aa" and request("tit")="" then
  strWhere = strWhere & " Name like '%"  & request("name") & "%'"
 else
   strWhere = strWhere & " and Name like '%"  & request("name") & "%'"
 end if
end if
...
if request("tit") <> "" then
 if request("sort")="aa" then
  ...
  strWhere = strWhere & " and Subject = '" & request("tit") & "'"
 end if
end if

if request("name") <> "" then
 if request("sort")="aa" and request("tit")="" then
  strWhere = strWhere & " Name = '" & request("name") & "'"
 else
   strWhere = strWhere & " and Name = '" & request("name") & "'"
 end if
end if

if request("replycount") <> "" then
	strWhere = strWhere & " replycount = " & request("replycount") & ""
end if

if request("timediff") <> "" then
	strWhere = strWhere & " now - LastNewsDate < " & request("timediff") & ""'天數
end if
...
%></td>
  <td height="14" width="15" align="center"><img src="images/post/<%=rs("R1")%>.gif" width=13 height=13 border=0></td>
...
<%sql="select * from Details where TitleID="&rs("TitleID")&" order by DetailID Desc"
  ...
  <%end if%>
  <td height="14" width="130" align="left"><font size=-1>
			...
			<%end if%>
  <p align="right" style="margin-top: 0; margin-bottom: 0"><%=rs("Name")%></p></font></td>
  ...
  <td height="14" width="33" align="center"><font size=-1><%=rs("replycount")%></font></td>
  ...
  <td height="14" width="130" align="left"><font size=-1>
			...
			<%end if%>
  <p align="right" style="margin-top: 0; margin-bottom: 0"><%=rs("lastposter")%></p></font></td>
  <td height="5" width="30" align="center"><font size=-1><a href="favorite.asp?id=<%=rs("TitleID")%>"><img src="images/eye.gif"border=0></a></font></td>
  </tr>
  <%
  rs.MoveNext
